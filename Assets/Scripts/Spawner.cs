...
public class Spawner : MonoBehaviour
{
    ...
    public Target SpawnTarget(int row)
    {
        ...
        Vector3 location = new Vector3((isDuck ? Random.Range(-10f, 10f) : Random.Range(-7f, 7f)), 0, 0);
        Vector3 position = (location.x < 0 ? new Vector3(-15, 0) : new Vector3(15, 0));

        Target target = GetFromPool(row, position);
        target.Set(location, Random.Range(0.5f, 1f));
        return target;
    }
